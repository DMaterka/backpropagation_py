...
class TestStorage(unittest.TestCase):
    ...
    def test_first_layer_bias_equals_read_one(self):
        np_test.assert_allclose(self.net.getLayer(0).getBias().getWeights(),
                         self.loaded_net.getLayer(0).getBias().getWeights())

    def test_first_layer_values_equals_read_one(self):
        np_test.assert_allclose(self.net.getLayer(0).getValues(), self.loaded_net.getLayer(0).getValues())
    
    def test_first_layer_sums_equals_read_one(self):
        np_test.assert_allclose(self.net.getLayer(0).getSums(), self.loaded_net.getLayer(0).getSums())

    def test_second_layer_bias_equals_read_one(self):
        # test if first layer bias are the same
        np_test.assert_allclose(self.net.getLayer(1).getBias().getWeights(),
